...
func (p *PoiService) ListPOIByH3Cells(ctx context.Context, in *poi_v1.ListPOIsByH3CellsRequest) (*poi_v1.ListPOIResponse, error) {
	...
	for _, index := range indexes {
		...
		cell := h3.Cell(c)
		if !cell.IsValid() {
			return nil, fmt.Errorf("invalid h3 index; %s", index)
		}
		if cell.Resolution() >= DATABASE_RESOLUTION {
			return nil, fmt.Errorf("h3 index resolution is too small(higher than 9 is not allowed); %s", index)
		}
	}

	res, err := p.Database.Queries.ListPOIsByH3Cells(ctx, sqlc_db.ListPOIsByH3CellsParams{
